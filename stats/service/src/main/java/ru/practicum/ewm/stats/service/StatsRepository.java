...
public interface StatsRepository extends JpaRepository<StatEntry, Long> {
    ...
    List<StatResult> findUris(LocalDateTime start, LocalDateTime end, List<String> uris);
    ...
    @Query("select new ru.practicum.ewm.stats.service.model.StatResult(se.app, se.uri, count(se.uri)) " +
            ...
            "group by se.app, se.uri " +
            "order by count(se.uri) desc")
    List<StatResult> findAllUris(LocalDateTime start, LocalDateTime end);
    ...
    @Query("select new ru.practicum.ewm.stats.service.model.StatResult(se.app, se.uri, count(distinct se.ip)) " +
            ...
            "group by se.app, se.uri " +
            "order by count(se.uri) desc")
    List<StatResult> findUrisAndUniqueIP(LocalDateTime start, LocalDateTime end, List<String> uris);
    ...
    @Query("select new ru.practicum.ewm.stats.service.model.StatResult(se.app, se.uri, count(distinct se.ip)) " +
            ...
            "group by se.app, se.uri " +
            "order by count(se.uri) desc")
    List<StatResult> findAllUrisAndUniqueIP(LocalDateTime start, LocalDateTime end);
}